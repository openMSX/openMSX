...
set token1 [list "" "END" "FOR" "NEXT" "DATA" "INPUT" "DIM" "READ" "LET" "GOTO" "RUN" "IF" "RESTORE" "GOSUB" "RETURN" "REM" "STOP" "PRINT" "CLEAR" "LIST" "NEW" "ON" "WAIT" "DEF" "POKE" "CONT" "CSAVE" "CLOAD" "OUT" "LPRINT" "LLIST" "CLS" "WIDTH" "ELSE" "TRON" "TROFF" "SWAP" "ERASE" "ERROR" "RESUME" "DELETE" "AUTO" "RENUM" "DEFSTR" "DEFINT" "DEFSNG" "DEFDBL" "LINE" "OPEN" "FIELD" "GET" "PUT" "CLOSE" "LOAD" "MERGE" "FILES" "LSET" "RSET" "SAVE" "LFILES" "CIRCLE" "COLOR" "DRAW" "PAINT" "BEEP" "PLAY" "PSET" "PRESET" "SOUND" "SCREEN" "VPOKE" "SPRITE" "VDP" "word" "CALL" "TIME" "KEY" "MAX" "MOTOR" "BLOAD" "BSAVE" "DSKO$" "SET" "NAME" "KILL" "IPL" "COPY" "CMD" "LOCATE" "TO" "THEN" "TABC" "STEP" "USR" "FN" "SPCL" "NOT" "ERL" "ERR" "STRING$" "USING" "INSRT" "" "VARPTR" "CSRLIN" "ATTR$" "DSKI$" "OFF" "INKEY$" "POINT" ">" "=" "<" "+" "-" "*" "/" "^" "AND" "OR" "XOR" "EQV" "IMP" "MOD" "\\" "" "" "{escape-code}"]
set token2 [list "" "LEFT$" "RIGHT$" "MID$" "SGN" "INT" "ABS" "SQR" "RND" "SIN" "LOG" "EXP" "COS" "TAN" "ATN" "FRE" "INP" "POS" "LEN" "STR$" "VAL" "ASC" "CHR$" "PEEK" "VPEEK" "SPACE$" "OCT$" "HEX$" "LPOS" "BIN$" "CINT" "CSNG" "CDBL" "FIX" "STICK" "STRIG" "PDL" "PAD" "DSKF" "FPOS" "CVI" "CVS" "CVD" "EOF" "LOC" "LOF" "MKI$" "MK$" "MKD$" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" "" ""]
...
#Check for listing
if {[word $addr]==0} {return "No Listing In Memory"}

#Initial Line Number
append listing [getLineNumber $addr];incr addr 4

#Loop through the memory 
	while {$go==1} {
		set tok ""
		if {[byte $addr]>0x80 && [byte $addr]<0xff} {set tok [lindex $token1 [expr [byte $addr]-0x80]]} else {
			if {[byte $addr]==0xff} {incr addr;set tok [lindex $token2 [expr [byte $addr]-0x80]]} else {
				if {[byte $addr]==0x3a} {
					set forward 0
					incr addr;
	...
	incr addr
	append listing $tok
	if {[word $addr]==0 || $addr>=0xffff} {set go 0}
	}
return $listing
}