...
void MSXMegaRom::writeMem(word address, byte value, Emutime &time)
{
  ...
  switch (mapperType){
    ...
    case 2: //   --==**>> KONAMI5 8kB cartridges <<**==--
      if ((address < 0x5000) || (address >0xB000) || ((address & 0x1FFF)!=0x1000)) return;
      // Enable/disable SCC
      if (address == 0x9000 ) { // 0x9000 is used as SCC switch
	enabledSCC=(value==0x3F)?true:false;
      }
      /* change internal mapper*/
    ...
    case 4: //   --==**>> ASCII 8kB cartridges <<**==--
      // TODO: As I understand corectly from fmsx this mapper changes if if not currently selected ??
