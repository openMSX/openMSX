...
TEST_CASE("FilePoolCore")
{
...

	{
		...
		// lookup, success
		{
			auto [file, fname] = pool.getFile(FileType::ROM, Sha1Sum("7e240de74fb1ed08fa08d38063f6a6a91462a815"));
			CHECK(file.is_open());
			CHECK(fname == one_of(tmp + "/a", tmp + "/a2"));
		}
		// lookup, not (yet) present
		{
			auto [file, fname] = pool.getFile(FileType::ROM, Sha1Sum("f36b4825e5db2cf7dd2d2593b3f5c24c0311d8b2"));
			CHECK(!file.is_open());
		}
		// lookup, success
		{
			auto [file, fname] = pool.getFile(FileType::ROM, Sha1Sum("5cb138284d431abd6a053a56625ec088bfb88912"));
			CHECK(file.is_open());
			CHECK(fname == tmp + "/b");
		}
		...
		{
			auto [file1, fname1] = pool.getFile(FileType::ROM, Sha1Sum("5cb138284d431abd6a053a56625ec088bfb88912"));
			CHECK(!file1.is_open());
			auto [file2, fname2] = pool.getFile(FileType::ROM, Sha1Sum("aa6878b1c31a9420245df1daffb7b223338737a3"));
			CHECK(file2.is_open());
			CHECK(fname2 == tmp + "/b");
		}
		...
		createFile(tmp + "/b",  "BBB"); // aa6878b1c31a9420245df1daffb7b223338737a3
		{
			auto [file, fname] = pool.getFile(FileType::ROM, Sha1Sum("aa6878b1c31a9420245df1daffb7b223338737a3"));
			CHECK(file.is_open());
			CHECK(fname == tmp + "/b");
		}
		// remove file
		...
		// calc sha1 of cached file
		{
			...
			File file(fname);
			CHECK(file.is_open());
			auto sum = pool.getSha1Sum(file, fname);
