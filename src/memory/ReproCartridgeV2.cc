...
void ReproCartridgeV2::setVolume(EmuTime::param time, byte value)
{
	...
	scc    .setSoftwareVolume(SoundDevice::VolumeType(((volumeReg >> 0) & 7) / 2.0f), time);
	psg0xA0.setSoftwareVolume(SoundDevice::VolumeType(((volumeReg >> 3) & 7) / 2.0f), time);
	psg0x10.setSoftwareVolume(SoundDevice::VolumeType(((volumeReg >> 6) & 3) / 2.0f), time);
}
