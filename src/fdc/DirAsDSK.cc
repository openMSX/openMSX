...
DirAsDSK::DirAsDSK(DiskChanger& diskChanger_, CliComm& cliComm_,
	...
	, syncMode(syncMode_)
	, diskFormat(getDiskFormat(diskChanger_.getNominalTracks(), diskChanger_.isDoubleSidedDrive()))
	, sectorsPerCluster(diskFormat.sectorsPerCluster)
	, sectorsPerDir(diskFormat.sectorsPerDir)
	, nofSectors((diskChanger_.isDoubleSidedDrive() ? 2 : 1) * SECTORS_PER_TRACK * diskChanger_.getNominalTracks())
