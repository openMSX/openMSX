...
std::string Paper::save(bool color) const {
  auto filename = FileOperations::getNextNumberedFileName(PRINT_DIR, "page",
                                                          PRINT_EXTENSION);
  if (color) {
    // Save colorBuf as RGB PNG (each row is sizeX*3 bytes)
    small_buffer<const uint8_t *, 4096> rowPointers(std::views::transform(
        xrange(sizeY), [&](size_t y) { return &colorBuf[sizeX * 3 * y]; }));
    PNG::saveRGB(sizeX, rowPointers, filename);
  } else {
    small_buffer<const uint8_t *, 4096> rowPointers(std::views::transform(
        xrange(sizeY), [&](size_t y) { return &buf[sizeX * y]; }));
