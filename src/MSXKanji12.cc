...
byte MSXKanji12::readIO(word port, const EmuTime& time)
{
...
byte MSXKanji12::peekIO(word port, const EmuTime& /*time*/) const
{
	byte result;
	switch (port & 0x0F) {
		case 0:
...
void MSXKanji12::writeIO(word port, byte value, const EmuTime& /*time*/)
{
