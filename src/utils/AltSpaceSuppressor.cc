...
//
// The Windows control menu will pop up by default in a window when the 
// user presses LALT+SPACE or RALT+SPACE.  Programmatically, this shows 
// up to the application as a series of Windows messages:  the respective 
// WM_SYSKEYDOWN events for ALT and SPACE, followed by a WM_SYSCOMMAND 
// message whose default processing by the OS results in the control menu 
// itself appearing.
// 
// The fix, at the application level, is to handle that WM_SYSCOMMAND 
// message (effectively swallowing it) and thus to prevent it from being 
// forwarded to Win32's DefWindowProc.
// 
// We first attempted to fix this by interacting with SDL's messaging subsystem.
// The hope was that by calling SDL_SetEventFilter with a custom event 
// filter, we could intercept and silence the offending WM_SYSCOMMAND message.
// Unfortunately, SDL allows an application listening for SDL_SYSWMEVENT 
// events to see those messages, but it does _not_ allow the application 
// to indicate that the event was handled and should not be processed further.
// 
// More concretely, the SDL code as of 1.2.13 in SDL_dibevents.c (circa line 250)
// ignores the return value from the event filter for WM events and ultimately 
// forwards the message to DefWindowProc anyway.  Further, in 
...
// 
// We filed bug 686 (http://bugzilla.libsdl.org/show_bug.cgi?id=686) on the SDL developers.  
// If they respond positively, we may want to return to a solution along those lines.
// 
// Our second attempt at a fix, the one represented by the code below, 
// involves registering our own Windows proc, filtering out the offending 
// WM_SYSCOMMAND event, and letting the rest go through.
// 
