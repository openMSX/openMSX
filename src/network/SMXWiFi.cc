...
void SMXWiFi::reset(EmuTime time)
{
	...
	updateEndpoint(time);
	if (isRealEndpoint()) {
		// No endpoint switch happened, but the UART is back to its
		// default baud rate: re-apply the configuration to the port.
		applyParamsToReal(time);
	}
}
...
void SMXWiFi::plug(Pluggable& device, EmuTime time)
{
	...
	// Configure the port right away when an RS232Raw is plugged, so the
	// first byte is always sent at the current UART speed (baud) and
	// format (8N1): on Windows this reopens the port before any traffic
	// flows, on POSIX the open handle is reconfigured live.
	if (dynamic_cast<RS232Raw*>(&getPluggedRS232Dev())) {
		applyParamsToReal(time);
	}
	updateEndpoint(time);
}

void SMXWiFi::unplug(EmuTime time)
{
...
{
	// The SM-X WiFi UART has a fixed format (8 data bits, 1 stop bit, no
	// parity): only the baud rate is configurable (via the command
	// register), so these are no-ops. They are still required by the
	// RS232Connector interface.
}
...
{
	// Only bytes from the active endpoint enter the FIFO: while a
	// plugged-but-inactive device (e.g. RS232Tester, or an RS232Raw whose
...
void SMXWiFi::writeCommand(uint8_t value, EmuTime time)
{
	updateEndpoint(time);
	if (value <= 9) {
		uartSpeed = value;
		if (isRealEndpoint()) {
			...
			emulated->setBaudRate(baudRates[value]);
		}
	} else if (value == 20) {
		resetFIFO();
		emulated->resetFifo();
...
// The UART talks to the real ESP32 only when the connector holds an
// RS232Raw whose host serial port is actually open; otherwise the
// emulated ESP32 is the active endpoint: nothing is plugged, a
// non-RS232Raw device is plugged, or the port could not be opened (no
// port selected, port in use, ...).
bool SMXWiFi::isRealEndpoint() const
{
	auto* raw = dynamic_cast<const RS232Raw*>(&getPluggedRS232Dev());
	return raw && raw->isOpen();
}

// Re-evaluates the active endpoint and performs the switch when it
// changed: stale bytes are flushed, the emulated ESP32 is reset (it
// always starts clean after a migration) and, when switching to the real
// endpoint, the current UART configuration is pushed to the serial device
// (the RS232Raw port may have opened with its own defaults).
void SMXWiFi::updateEndpoint(EmuTime time)
{
	bool real = isRealEndpoint();
	if (real == realEndpointActive) return;
	realEndpointActive = real;
	resetFIFO();
	emulated->resetState();
	if (real) {
		applyParamsToReal(time);
...
// Pushes the current UART configuration to the plugged device: the baud
// rate (configurable via the command register) and the fixed format
// 8N1 with DTR/RTS deasserted.
void SMXWiFi::applyParamsToReal(EmuTime time) const
{
