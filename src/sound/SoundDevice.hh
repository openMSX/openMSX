...
public:
	 ...
	 *  false -> not muted
	 *  true  -> muted
	 * This method should only be used by the UI, internal muting must be
	 * done with setInternalMute()
	 */
	 ...
	 *  false -> not muted
	 *  true  -> muted
	 * This method should only be used by the UI, checking if the devices
	 * needs to be mixed must be done with isInternalMute()
	 */
	bool isMuted() const;
...
protected:
	 ...
	 * produce sound for the moment it can be muted.
	 *
...
public:
	 ...
	 * native format (e.g. 16-bit signed).  If a device is muted (method
	 * isInternalMuted()) this method should return a null-pointer.
	 *
