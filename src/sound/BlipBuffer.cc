...
constexpr auto impulses = [] {
	...
	//for (auto i : xrange(HALF_SIZE)) { msvc bug
	...
	//for (auto i : xrange(BLIP_RES)) { msvc bug
	...
	double sum = 0.0;
	double next = 0.0;
	//for (auto i : xrange(IMPULSES_SIZE)) { msvc bug
	...
	//for (auto phase : xrange(BLIP_RES)) { msvc bug
	for (int phase = 0; phase < BLIP_RES; ++phase) {
		...
		//for (auto i : xrange(BLIP_IMPULSE_WIDTH / 2)) { msvc bug
