...
uint8_t MSXPPI::readIO(uint16_t port, EmuTime::param time)
{
	return i8255.read(port & 0x03, time);
}

uint8_t MSXPPI::peekIO(uint16_t port, EmuTime::param time) const
{
	return i8255.peek(port & 0x03, time);
}

void MSXPPI::writeIO(uint16_t port, uint8_t value, EmuTime::param time)
{
...
uint8_t MSXPPI::readA(EmuTime::param time)
{
	return peekA(time);
}
uint8_t MSXPPI::peekA(EmuTime::param /*time*/) const
{
...
void MSXPPI::writeA(uint8_t value, EmuTime::param /*time*/)
{
	getCPUInterface().setPrimarySlots(value);
}

uint8_t MSXPPI::readB(EmuTime::param time)
{
	return peekB(time);
}
uint8_t MSXPPI::peekB(EmuTime::param time) const
{
...
void MSXPPI::writeB(uint8_t /*value*/, EmuTime::param /*time*/)
{
	// probably nothing happens on a real MSX
}

uint4_t MSXPPI::readC1(EmuTime::param time)
{
	return peekC1(time);
}
uint4_t MSXPPI::peekC1(EmuTime::param /*time*/) const
{
	return 15; // TODO check this
}
uint4_t MSXPPI::readC0(EmuTime::param time)
{
	return peekC0(time);
}
uint4_t MSXPPI::peekC0(EmuTime::param /*time*/) const
{
	return 15; // TODO check this
}
void MSXPPI::writeC1(uint4_t value, EmuTime::param time)
{
	if ((prevBits ^ value) & 1) {
...
void MSXPPI::writeC0(uint4_t value, EmuTime::param /*time*/)
{
	selectedRow = value;
}
