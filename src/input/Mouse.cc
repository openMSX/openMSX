...
void Mouse::write(uint8_t value, EmuTime::param time)
{
	if (mouseMode) {
		...
		switch (phase) {
		case PHASE_XHIGH1:
			if ((value & STROBE) == 0) phase = PHASE_XLOW1;
			break;
		case PHASE_XLOW1:
			if ((value & STROBE) != 0) phase = PHASE_YHIGH1;
			break;
		case PHASE_YHIGH1:
			if ((value & STROBE) == 0) phase = PHASE_YLOW1;
			break;
		case PHASE_YLOW1:
			if ((value & STROBE) != 0) phase = PHASE_XHIGH2;
			break;
		case PHASE_XHIGH2:
			if ((value & STROBE) == 0) phase = PHASE_XLOW2;
			break;
		case PHASE_XLOW2:
			if ((value & STROBE) != 0) phase = PHASE_YHIGH2;
			break;
		case PHASE_YHIGH2:
			if ((value & STROBE) == 0) phase = PHASE_YLOW2;
			break;
		case PHASE_YLOW2:
			if ((value & STROBE) != 0) {
				phase = PHASE_XHIGH1;
