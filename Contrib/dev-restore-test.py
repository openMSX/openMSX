...
def main():
    ...
    for name, mapper in [('flash', 'ASCII16-X'), ('plain', 'ASCII16'), ('ips', 'ASCII16-X'), ('gzip', 'ASCII16-X')]:
        ...
        try:
            ...
            normal = 0x33 if patch else (0x11 if mapper == 'ASCII16-X' else 0x22)
            assert int(emu.command('debug read memory 0x5000')) == normal
            ...
            assert emu.command(snapshot) == runtime
            assert int(emu.command('debug read memory 0x5000')) == expected
            if mapper == 'ASCII16-X':
                assert emu.command('debug read memory 0x8100') == '90'
            ...
            reports[name] = 'PASS: runtime/banks/VRAM retained, new graphics decoded, Flash save preserved, ordinary restore unchanged'
