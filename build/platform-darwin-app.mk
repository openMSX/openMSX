...
ifeq ($(3RDPARTY_INSTALL_DIR),)
3RDPARTY_INSTALL_DIR:=$(shell sdl-config --static-libs | sed -e "s%^\(.* \)*\(/.*\)/lib/libSDL\.a.*$$%\2%" 2>> /dev/null)
else
...
ifneq ($(shell test -x $(3RDPARTY_INSTALL_DIR)/bin/sdl-config ; echo $$?),0)
$(error Cannot execute sdl-config)
endif
endif

PNG_CFLAGS:=`$(3RDPARTY_INSTALL_DIR)/bin/libpng-config --cflags 2>> $(LOG)`
PNG_LDFLAGS:=$(3RDPARTY_INSTALL_DIR)/lib/libpng.a
# TODO: In theory this should return the flags for static linking,
#       but in libpng 1.2.8 it just returns the dynamic link flags.
#PNG_LDFLAGS:=`$(3RDPARTY_INSTALL_DIR)/bin/libpng-config --static --libs 2>> $(LOG)`
PNG_RESULT:=`$(3RDPARTY_INSTALL_DIR)/bin/libpng-config --version`

SDL_CFLAGS:=`$(3RDPARTY_INSTALL_DIR)/bin/sdl-config --cflags 2>> $(LOG)`
SDL_LDFLAGS:=`$(3RDPARTY_INSTALL_DIR)/bin/sdl-config --static-libs | sed -e \"s/-L[^ ]*//g\" 2>> $(LOG)`
SDL_RESULT:=`$(3RDPARTY_INSTALL_DIR)/bin/sdl-config --version`
...
SDL_IMAGE_LDFLAGS:=$(SDL_LDFLAGS) $(PNG_LDFLAGS) \
	$(3RDPARTY_INSTALL_DIR)/lib/libSDL_image.a \
	$(shell grep -q "\-ljpeg" $(3RDPARTY_INSTALL_DIR)/lib/libSDL_image.la 2>> /dev/null \
		&& echo "$(3RDPARTY_INSTALL_DIR)/lib/libjpeg.a")

GLEW_CFLAGS:=-I$(3RDPARTY_INSTALL_DIR)/include
GLEW_LDFLAGS:=$(3RDPARTY_INSTALL_DIR)/lib/libGLEW.a
